...
public class MemberDAO {
	...
	public void insertMember(Member m) throws SQLException {
		final String SQL = "INSERT INTO [MidTermProject].[dbo].[member]([account], [password], [level], [mName], [mPhone]) VALUES(?, ?, ?, ?, ?)";

		PreparedStatement preState = conn.prepareStatement(SQL);
		preState.setString(1, m.getAccount());
		preState.setString(2, m.getPassword());
		preState.setString(3, m.getLevel());
		preState.setString(4, m.getmName());
		preState.setString(5, m.getmPhone());

		preState.executeUpdate();
		preState.close();
	}

	public Member findMemberByAccountAndPassword(String account, String password) throws SQLException {
		final String SQL = "SELECT * FROM [MidTermProject].[dbo].[member] WHERE [account] COLLATE Latin1_General_CS_AS = ? AND [password] COLLATE Latin1_General_CS_AS = ?";
		PreparedStatement preState = conn.prepareStatement(SQL);
